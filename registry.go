...
func getRemoteDigest(image string) (string, error) {
	digest, _, err := getRemoteDigestCandidates(image)
	return digest, err
}

func getRemoteDigestCandidates(image string) (string, []string, error) {
	...
	if code == 200 {
		return resolveManifestDigests(digest, body, headers)
	}
	if code != 401 {
		return "", nil, fmt.Errorf("registry returned %d", code)
	}
	...
	if code != 200 {
		return "", nil, fmt.Errorf("registry returned %d after auth", code)
	}
	return resolveManifestDigests(digest, body, headers)
}

func resolveManifestDigests(digest string, body []byte, headers http.Header) (string, []string, error) {
	...
	platformDigest, err := resolvePlatformDigest(body)
	if err != nil {
		return digest, []string{digest}, nil
	}
	return digest, uniqueDigests(digest, platformDigest), nil
}
...
func resolvePlatformDigest(body []byte) (string, error) {
